...
- The audit revalidates exact US standard-audiobook identities using the same current-work gate. Its retained release-date counts remain operational; only a reviewed audio manifest can establish series coverage or currency. Publisher-only editions remain unverified until the loader supports their proof.
