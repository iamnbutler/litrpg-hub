...
export function buildCatalog(db: Database.Database, now = new Date()): Catalog {
	const rows = db.prepare(`SELECT b.*, s.title AS series_title,
		...
		a.assessment_json, c.assessment_json AS content_json FROM books b LEFT JOIN series s ON s.id = b.series_id
	...
	const books: CatalogBook[] = rows.filter(r => r.title && r.title !== 'Untitled').map(row => {
		const book: CatalogBook = {
			...
			edition: /\b(?:box(?:ed)?\s*set|omnibus|collection|books?\s*\d+\s*[-–]\s*\d+)\b/i.test(`${row.title} ${row.subtitle ?? ''}`) ? 'collection' :
				/\b(?:dramatized|dramatised|graphic\s*audio|full.cast)\b/i.test(`${row.title} ${row.subtitle ?? ''}`) ? 'dramatized' : 'audiobook',
		...
		if (book.coverAssessment) {
			// Cover evidence can establish sexualized marketing, never on-page sex or harem.
			...
			if (row.content_json) {
				const cached = JSON.parse(row.content_json) as ContentAssessment;
				if (cached.inputHash === contentAssessmentHash(book, book.coverAssessment)) {
		...
		for (const [key, override] of Object.entries(overrides[row.id] ?? {})) {
			if (!(key in book.content) || !override || !['present', 'absent', 'unknown'].includes(override.verdict) || !override.note?.trim()) throw new Error(`Invalid content override for ${row.id}/${key}`);
			book.content[key as keyof CatalogBook['content']] = { ...override, source: 'manual', confidence: 1 };
		}
		const genre = book.assessment?.genre;
		book.scope = book.subgenres.length > 0 && !(genre?.value === 'unrelated' && genre.confidence >= 0.8) ? 'indexed' : 'review';
