...
<style>
.book-row { display:grid; grid-template-columns:50px minmax(0,1fr) 100px 114px 170px; align-items:center; gap:18px; min-width:0; padding:10px; border-bottom:1px solid var(--line); } .book-row:last-child { border-bottom:0; } .book-row:hover { background:var(--surface-hover); }
...
h3 { margin:0 0 5px; font:500 15px/1.4 var(--serif); } h3 button { padding:0; border:0; background:none; text-align:left; font:inherit; display:-webkit-box; -webkit-line-clamp:2; line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; } h3 button:hover { color:var(--accent); text-decoration:underline; }
.author { font-size:13px; margin:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; } .author span,.genres { color:var(--muted); } .genres,.match-reason { font-family:var(--mono); font-size:10px; margin:4px 0 0; line-height:1.4; } .match-reason { color:var(--green); }
...
.book-actions { display:flex; align-items:start; flex-direction:column; gap:6px; } .save-book { border:1px solid var(--line); border-radius:4px; background:var(--surface); color:var(--ink); padding:5px 9px; min-width:112px; font-size:11px; } .save-book:hover,.save-book.saved { background:var(--surface-hover); color:var(--green); } .similar-book { border:0; background:none; color:var(--green); padding:0; font-size:11px; } .similar-book:hover { text-decoration:underline; }
...
.grid .book-cover { width:100%; height:auto; aspect-ratio:1; border-radius:3px; overflow:hidden; background:var(--surface); }
...
.grid .save-book:hover,.grid .save-book.saved { background:var(--surface-hover); }
